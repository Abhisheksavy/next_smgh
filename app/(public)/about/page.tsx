...
const About = async () => {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_APP_BASE_URL}/pages/about`,
    {
      next: {
        revalidate: 10,
      },
  ...
  return (
    <div>
      ...
      {aboutPageData?.qualityCare && (
        <QualityCare qualityCare={aboutPageData?.qualityCare} />
      )}
      <MissionVision mission={aboutPageData?.mission} />

      {data?.leadership && <LeaderShip leadership={data?.leadership} />}
      {aboutPageData?.annualReport && (
        <AnnualReport annualReport={aboutPageData?.annualReport} />
      )}
