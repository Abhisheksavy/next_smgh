    ...
    <div className="form-container bg-primary/3 section-padding spacing-x rounded-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        {config.behalfInfo && (
          <div className="space-y-6">
            <h3 className="text-xl font-semibold text-primary mb-4">
              Information on Behalf of Patient
            </h3>
            <div className="grid md:grid-cols-2 gap-6">
              {Object.entries(config.behalfInfo).map(([key, field]) => {
                ...
                const isRequired =
                  field.required === "true" || field.required === true;
                const shouldShow =
                  key === "submittingForSomeoneElse" ||
                  submittingForSomeoneElse === "yes";

                if (!shouldShow && key !== "submittingForSomeoneElse")
                  return null;
                ...
                return (
                  <div
                    key={fieldKey}
                    className={
                      key === "submittingForSomeoneElse" ? "md:col-span-2" : ""
                    }
                  >
                    <label className="text-primary text-sm font-medium mb-2 block">
                      {field.label}
